...
async def get_file_ids(client: Client | bool, db_id: str, multi_clients, message) -> Optional[FileId]:
    ...
    if client_id_str not in file_id_info:
        fallback_file_id = next(iter(file_id_info.values()), None) if file_id_info else None
        if fallback_file_id:
            file_id_info[client_id_str] = fallback_file_id
    ...
    file_id = FileId.decode(file_id_info[client_id_str])
